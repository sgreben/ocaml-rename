...
let check_type_decl env loc id row_id newdecl decl rs rem =
  ...
  Includemod_hack.type_declarations env id newdecl decl;
  ...
  let rec merge env sg namelist row_id =
    ...
    | (Sig_module(id, md, rs) :: rem, [s], Pwith_module (_, lid'))
      when Ident.name id = s ->
        ...
        ignore(Includemod_hack.modtypes env newmd.md_type md.md_type);
  ...
  let rec check_incl first_time n env s =
    if n > 0 then begin
      ...
      let check_inclusion (id, id_loc, mty_decl, modl, mty_actual, attrs, loc) =
        ...
        let coercion =
          try
            Includemod_hack.modtypes env mty_actual' mty_decl'
...
let package_subtype env p1 nl1 tl1 p2 nl2 tl2 =
  ...
  try Includemod_hack.modtypes env mty1 mty2 = Tcoerce_none
...
let wrap_constraint env arg mty explicit =
  let coercion =
    try
      Includemod_hack.modtypes env arg.mod_type mty
    with Includemod.Error msg ->
      ...
      let md =
        if alias && not (Env.is_functor_arg path env) then
          ...
          Mty_alias p1 when not alias ->
            ...
            let mty = Includemod_hack.expand_module_alias env [] p1 in
            { md with
  ...
  | Pmod_apply(sfunct, sarg) ->
      ...
      begin match Env.scrape_alias env funct.mod_type with
        Mty_functor(param, mty_param, mty_res) as mty_functor ->
          ...
          let coercion =
            try
              Includemod_hack.modtypes env arg.mod_type mty_param
            with Includemod.Error msg ->
...
let type_implementation sourcefile outputprefix modulename initial_env ast =
  ...
  if !Clflags.print_types then begin
    ...
    if Sys.file_exists sourceintf then begin
      ...
      let coercion =
        Includemod_hack.compunit initial_env sourcefile sg intf_file dclsig in
      ...
      let coercion =
        Includemod_hack.compunit initial_env sourcefile sg
...
let package_units initial_env objfiles cmifile modulename =
  ...
  if Sys.file_exists mlifile then begin
    ...
    Includemod_hack.compunit initial_env "(obtained by packing)" sg mlifile dclsig
  end else begin
    (* Determine imports *)
...
let report_error ppf = function
  ...
  | Not_included errs ->
      fprintf ppf
        "@[<v>Signature mismatch:@ %a@]" Includemod_hack.report_error errs
  ...
  | With_mismatch(lid, explanation) ->
      fprintf ppf
        "@[<v>\
           ...
           %a@]"
        longident lid Includemod_hack.report_error explanation
